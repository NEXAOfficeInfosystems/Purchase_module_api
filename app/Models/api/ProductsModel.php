...
class ProductsModel extends Model
{
    ...
    protected $allowedFields = [
        ...
        'unit_id',
        'sku',
        'thumbnail_image', // Include thumbnail_image field
        'gallery_images',
        'product_tags',
        'short_description',
        'description',
    ...
    public function searchproduct($search = "")
    {
        ...
        if ($search == "") {
            $sql = "SELECT p.* 
                    FROM product_categories as pc, categories as c, products as p 
            ...
            $sql = "SELECT p.* 
                    ...
                    WHERE p.id = pc.product_id 
                    AND c.id = pc.category_id 
                    AND (LOWER(c.name) LIKE LOWER('$search%') OR LOWER(p.name) LIKE LOWER('$search%')) 
                    AND p.is_published = 1";
        }
